...
class RectangularRoom:
    ...
    def intersects(self, other: RectangularRoom) -> bool:
        #return true if this room overlaps with another RecRoom
        return (
            self.x1 <= other.x2
...
def generate_dungeon(
    ...
    for r in range(max_rooms):
        ...
        #run through other rooms to see if they interact with this one
        if any(new_room.intersects(other_room) for other_room in rooms):
            continue #this room intersects, so go to next attempt
        #if no intersections -> room is valid

        #dig out this rooms inner area
        dungeon.tiles[new_room.inner] = tile_types.floor

        if len(rooms) == 0:
            #the first room the player starts
